...
class Pool:
    ...
    A pool in Kea is a range in NAV because it contains all IP addresses between
    its declared first IP address and its declared last IP address.
    """
...
class Client:
    ...
    def __init__(
        ...
        self._http_basic_user: Optional[str] = http_basic_username
    ...
    def _create_session(self) -> Session:
        ...
        if self._http_basic_user is not None and self._http_basic_password is not None:
    ...
    def _pools_of_subnet(self, subnet: dict) -> Iterator[Pool]:
        ...
        for pool in subnet.get("pools", []):
            try:
                ...
                _logger.error(
                    'Could not parse Kea pool in subnet %d from %s, skipping Kea pool... '
                    '(make sure every Kea pool has "pool-id" and "pool" configured in the '
    ...
    def _log_consistency_with_upstream_pools(self, local_pools: list):
        ...
        if sorted(local_pools) != sorted(upstream_pools):
            _logger.warning(
                ...
                "be associated with the wrong Kea pool (and subsequently wrong range in "
                "NAV)."
            )
